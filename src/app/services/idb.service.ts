  ...
  async installArabicPacks() {
    ...
    try {
      if (await this.validate(arabicEdition)) {
        ...
        const mappedQuran = data.ayahs.map((ayah, index) => {
          const [arabicText, numberInSurah] = ayah;
          return { arabicText, numberInSurah, _key: (index + 1).toString() };
        });
        await this.db
          .collection(arabicEdition)
          .set(mappedQuran, { keys: true });
        await Timer(6000);
        return true;
      }
    } catch (error) {
  ...
  async installTranslationPacks() {
    for (let translation of this.translations) {
      if (await this.validate(translation)) {
        ...
        await this.db.collection(translation).set(mappedQuran, { keys: true });
        await Timer(6000);
      }
    }
  }
